...
| **1.9** | Jan 2025 | **Provider iteration — `for_each` on provider configurations** (one instance per region/account from a map/set); **`-exclude` flag** — inverse of `-target`, mutually exclusive with it; **`-show-sensitive`** to unmask sensitive values; `-consolidate-warnings` / `-consolidate-errors`. | Both **OpenTofu-only** as of Terraform 1.15. A resource's `for_each` must be a *subset* of its provider's. `-exclude-file`/`-target-file` came in 1.10. |
