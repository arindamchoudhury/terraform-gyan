...
| 1.10 | **`deprecated` on input variables and output values**, warning at the call site. **Short-circuiting `&&` and <code>&#124;&#124;</code>**. `element` accepts negative indices. `moved` supports cross-type moves with automatic state migration. `removed` blocks can carry `lifecycle` and `provisioner` configuration. `module` `version` accepts `null`. |
