...
!!! info "OpenTofu — the `-exclude` flag (OpenTofu 1.9)"
    OpenTofu added a **`-exclude`** flag (OpenTofu **1.9**) — the inverse of `-target`: plan/apply *everything except* the named addresses. Terraform still has **no `-exclude`**, verified absent from `internal/command/arguments/` at **v1.16**. `-target` and `-exclude` are mutually exclusive. Portable code should avoid depending on either. (See [[version-facts]], [[feature-coverage-matrix]].)

    Two details verified in source on 2026-08-13, neither of which the summary above carries. The exclusion is enforced at **argument parsing**, before planning, and **1.10 widened it from the two flags to the two families** on the commit that added `-target-file`/`-exclude-file` (`556ba25638`), so any target-side option combined with any exclude-side option now fails with *"The target and exclude planning options are mutually-exclusive. Each plan must use either only the target options or only the exclude options."* At 1.9 the message was the narrower *"-target and -exclude flags cannot be used together. Please remove one of the flags"*. The practical consequence: a plan is allow-listed or deny-listed, never both. (See [[opentofu-release-feature-map]].)
