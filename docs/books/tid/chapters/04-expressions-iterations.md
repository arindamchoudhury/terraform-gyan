...
Takes a **map**, **object**, or **set** (not an integer) and exposes **`each`** with `each.key` and `each.value`. For a set, key and value are the same element. Each instance gets its own configuration:
