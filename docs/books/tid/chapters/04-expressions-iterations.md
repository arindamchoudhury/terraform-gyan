...
!!! danger "Branch types must match — but the book's 'evaluates both results' is outdated"
    ...
    So guard the branch you might **take**, not "both": `try(module.nat_instance[0].ip, null)` protects the case where `use_instance` is actually true. The book's claim was true only **before Terraform 0.12.0** — the HCL2 rewrite (May 2019) made branch evaluation lazy (bug [#15605](https://github.com/hashicorp/terraform/issues/15605), fixed in 0.12.0-alpha1), so it's been outdated for ~6 years. OpenTofu 1.12.4 behaves the same (it forked from TF 1.5.x, post-0.12). Full evidence: [[conditional-branch-evaluation]].
