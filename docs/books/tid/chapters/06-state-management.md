...
!!! info "S3 locking no longer needs DynamoDB — the book's Table 6.1 is out of date"
    The book says the `s3` backend "requires DynamoDB for locking, which makes it unsuitable for Wasabi or other S3-emulation systems." Both tools now lock **natively** using S3 **conditional writes** (a `.tflock` object in the bucket), enabled with **`use_lockfile = true`** — no DynamoDB table. This removes the Wasabi/emulator caveat, since only S3 conditional-write support is required.

    **The two tools are not at the same point, and this note previously said they were.** Terraform introduced native locking in **1.10** (2024-11-27) as opt-in, taking *both* locks when DynamoDB was also configured, and made it **GA in 1.11** (2025-02-27) — which is also where `dynamodb_table` was actually **deprecated**. OpenTofu shipped `use_lockfile` in its **1.10** (June 2025), about seven months after Terraform's first release of it; matching version numbers on different schedules make this easy to get backwards.

    **And `dynamodb_table` is deprecated only on Terraform.** Verified in the backend schemas: Terraform v1.15.0 marks it `Deprecated: true` and emits a diagnostic steering you to `use_lockfile`; OpenTofu's schema on `main` carries no deprecation and no diagnostic. So "migrate off DynamoDB" is Terraform advice — on OpenTofu it remains a fully supported option, even though `use_lockfile` is the better default there too. Either tool accepts both during migration. See [[version-facts]], [[release-feature-map]], [[opentofu-release-feature-map]].
...
- **Backends** are built-in; the `local` one is dev-only. Pick by existing tech; use **partial config** + `-backend-config`; migrate with `-migrate-state`. **S3 now locks natively via `use_lockfile` (Terraform 1.10, GA 1.11; OpenTofu 1.10) — `dynamodb_table` is deprecated on Terraform, still supported on OpenTofu.**
...
- S3-native locking (Terraform 1.10, GA 1.11; OpenTofu 1.10) — PR <https://github.com/hashicorp/terraform/pull/35661> · <https://opentofu.org/docs/language/settings/backends/s3/>
