...
**Edges are resource-level, and `-target` is where you feel it.** The verification above found `nodeExpandPlannableResource` created for *every* managed resource, deciding the expansion before instances get their own nodes. [[tut-resource-targeting]] is the observable consequence: targeting a single `count` instance, `-target="aws_s3_object.objects[2]"`, planned the replacement of **all four** `random_pet.object_names` instances, because `objects[2] → object_names[2]` is not an edge Terraform has — `objects → object_names` is. The page states the rule in its own words: *"Terraform calculates resource dependencies for the entire resource."*

So an instance-level target gives instance-level precision on the target itself and none at all upstream of it. The same page supplies the direction rule that makes targeting a subgraph selection rather than a filter: it pulls in everything the target *depends on*, and nothing that *depends on* the target.
