...
- **`mv` moves state and leaves the configuration behind; `moved` moves both but only within one file.** [[tut-state-cli]] shows the gap concretely: a successful `terraform state mv -state-out=…` leaves the destination one plan away from `# … will be destroyed # (because … is not in configuration)`, fixed only by pasting the `resource` block in by hand. [[tut-move-config]]'s `moved` block has no such step, and no cross-file equivalent — which is why [[tf-state-refactor]] calls the two-file `state mv` legacy and prefers `removed` + `import` for new migrations. Same underlying fact throughout: **an address is an identity**, and "not in configuration" is indistinguishable from a deletion.
...
- Need to *operate* on state — adopt, rename, forget, split, or repair it? → [Book Ch 16 — State management operations](../book/ch16-state-operations.md), which puts `import`, `moved`, `removed`, drift and the `state` CLI family in one place with measured transcripts on both engines.
- Need to understand what is *in* the file? → **TID Ch6** ([[06-state-management]]).
...
- Want to know which CLI operations write state at all? → [[tf-cli-state]], the section index for the write side. It names three consequences of getting it wrong (cost, productivity, security), makes the state backup unconditional, and its sidebar is the full toolkit inventory. It also still fronts `taint` as the way to re-create a resource, which `-replace` superseded.
- Unsure what an address without an index selects? → [[tf-resource-addressing]]. A bare `aws_instance.web` under `count = 4` is **all four**, and `module.foo` is every resource in every instance of that module call. Which commands accept that breadth differs per command, which is the point.
- About to run any `terraform state` subcommand? → [[tf-cmd-state]] first. Every modifying subcommand writes a backup that **cannot** be disabled, remote state is operated on exactly like local state but each read and write costs a network roundtrip, and those backups land on local disk either way.
...
- [[tf-cli-state]] — HCDocs, the *Manually Update State* section index: the counterpart to [[tf-cli-inspect]], and the source for `replace-provider` being classed as a move rather than a provider operation
- [[tf-resource-addressing]] — HCDocs, the address grammar every state command and refactoring block parses; source of the *one grammar, several parsers* rule and the two pre-0.13 legacy behaviours
- [[tf-cmd-state]] — HCDocs, the `state` subcommand hub: the forced-backup rule, the remote-state equivalence, and an index that omits `state push`
