...
- **code-style-and-tooling** — currently from: [[tf-style-guide]] (HashiCorp's rules: naming, ordering, file layout, version pinning) + [[code-styling]] (Anton Babenko's enforcement toolchain: `pre-commit`, `terraform fmt -check` in CI, `.editorconfig`, `terraform-docs`). Two sources — promotable; feeds B4/A3/I5. The interesting seam is that the two barely overlap: one states rules, the other automates them, and only the community book has a section-header convention.
