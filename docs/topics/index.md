...
- **refactoring** — currently from: TID Ch2 §2.9 + **TID Ch6 §6.5** (`moved`/`removed` blocks, `state rm`/`replace-provider`, hand-edit last-resort) + [[tf-state-refactor]] (the **cross-configuration** case: when to split, how to group, the `removed`+`import` migration recommended over legacy `state mv -state/-state-out`, empty plans on both sides as the proof); full treatment is TID Ch9.
