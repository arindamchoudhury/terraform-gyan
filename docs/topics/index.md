...
- **state** — currently from: [[terraform-intro]] + TID Ch2 §2.3.1 (backend/`cloud` blocks) + **TID Ch6** (full treatment: purpose of state, tfstate JSON, backends/workspaces/migration, `moved`/`removed`, drift, `terraform_remote_state`, state-only providers) + [[tf-state]] (HCDocs State overview: bindings as the primary purpose, the one-to-one rule and how `import`/`state rm` break it, `-json` integration points) + [[tf-state-purpose]] (the four why-state-exists arguments: real-world mapping and the failed tag prototype, **retained dependencies for destroy ordering**, the attribute cache as the optional part, syncing/locking). Promotable once TUR's state chapters land — feeds B9/I6/I7.
