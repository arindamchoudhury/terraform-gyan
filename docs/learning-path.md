...
3. **Book chapter — TID Ch 3–4** (~1 hr) — implicit vs explicit dependencies and how the graph is derived. An attribute reference *is* the dependency edge; see [[dependency-graph]] for the two inputs the DAG is built from, and why nothing warns when one is missing.
...
!!! note "📌 Output `type` (Terraform 1.15)"
    Terraform **1.15** added a `type` attribute to `output` blocks — outputs can now carry the same type constraints (and documentation value) variables always have. Prefer typing your module outputs. (See [[tf115-ot112-features]]; the full `output` argument spec is in [[tf-block-output]].)
...
3. **Book chapter — TID Ch 5** (~1 hr) — expression patterns used in real configs; the plan-time-known limits on computed values in `count`/`for_each` are the pointy end (Captured notes: [[05-terraform-plan]] §5.7 — see esp. *Calculated values and iterations*).
...
5. **Book chapter — TID Ch 5** (~1 hr) — the re-creation pitfall and how `for_each` keys avoid it. (Captured notes: [[05-terraform-plan]] — §5.7 *Calculated values and iterations* on the plan-time-known constraint, §5.2 on the resource graph the meta-args expand into.)
...
3. **Book chapter — TID Ch 5** (lifecycle) (~30 min) — real use cases and gotchas; `ignore_changes` as the fix for cascading `# forces replacement` (Captured notes: [[05-terraform-plan]] §5.7 *Cascading changes*).
...
> **Last updated:** 2026-07-13 — **Chapter 6 written — B6 (Input variables, outputs & locals) ✅** (`docs/book/ch06-input-variables-outputs-locals.md`). Blends nine sources — [[tf-input-variables]], [[tf-block-variable]], [[tf-locals]], [[tf-block-locals]], [[tf-outputs]], [[tf-block-output]], [[tut-variables]], [[tut-locals]], [[tut-outputs]] (plus [[tf-aws-manage]] and an A6 preview from [[tf-manage-sensitive-data]]) — into the module-as-function model: variables in, outputs out, locals internal. Two web searches run for current pitfalls; **verified against the source** that a `variable` block has **no mutually exclusive arguments** (a blog claim that `const` excludes `sensitive`/`ephemeral` is wrong — the official `block/variable` page says otherwise). Folded the `default = null` ≠ optional gotcha in as a pitfall. Milestone (one config → two environments via `-var-file`) is the lab, on S3 against the emulator. Coverage audit clean: secrets depth→A6, isolation→A7, `count`/`for_each`→I1, `optional()`→I3, functions/console→B7 all named-deferred. Beginner now B1–B6 done (6/31 chapters). No new glossary terms (all added during the note captures). · 2026-07-13 — **Source note [[tf-conditionals]]** (HCDocs "Conditional Expressions" captured): the ternary `cond ? a : b` plus the condition-building cookbook that feeds B7 and A2. **Both result branches must share a type** — else Terraform auto-converts to a common type (`var.example ? 12 : "hello"` → string), which the docs say to avoid via explicit `tostring()`. The cookbook: logical/comparison operators, `contains(set, x)`, `length(...) != 0` (better than `== []`/`!= []` for same-typed empty collections), `for` + `alltrue`/`anytrue`, and **`can(expr)`** — returns a bool for whether `expr` errors, letting you use `regex`/`tostring`/`tolist`/attribute access as conditions. Special objects: **`self`** (postcondition, instance under evaluation) and `each`/`count` in expanded blocks. New **Expressions** nav group mirroring the docs sidebar. Wired into B7 reference #2 (ternary + `can`) and A2 reference #2 (`self`/`can` for writing conditions); glossary gains a `can()` row and dual-sources the Conditional row. No topic status flipped. · 2026-07-13 — **Source note [[tut-outputs]]** (HCTut "Output data from Terraform" — Configuration Language collection #5): hands-on for [[tf-outputs]]/[[tf-block-output]]. The keeper is the **exact `sensitive` redaction matrix**: Terraform redacts on plan/apply/destroy and `terraform output` (all), but **not** when querying **by name** (`terraform output db_password` → plaintext), with `-json`, with `-raw`, or when a child output is used in root — and sensitive values sit in state as plain text (`grep` the tfstate). This sharpens the A6 redaction callout (previously only `-json`/`-raw`; now the by-name case too). Also the query workflow: `terraform output` / `<name>` / `-raw` (unquoted since 0.14 wraps strings) / `-json` (each entry carries `sensitive`/`type`/`value`), outputs stored in state so you must `apply` to record them even with no infra change. **Nav:** renumbered the Configuration Language collection to match the site's order (Resources 1, Variables 2, Sensitive variables **3 — not yet captured**, Locals 4, Outputs 5). Wired into B6 outputs clause and the A6 sensitive-redaction callout. No topic status flipped. · 2026-07-13 — **Source note [[tf-block-output]]** (HCDocs "`output` block reference" captured — closes the value-trio reference set with [[tf-block-variable]] and [[tf-block-locals]]): the full argument surface the how-to [[tf-outputs]] skips. `value` is the only required arg; adds **`type`** (1.15 output type constraint), **`depends_on`** (explicit dep, rarely needed — docs say comment it), **`deprecated`** (1.15, child-modules-only, only *consumers* see the warning, `ignore_nested_deprecations` suppresses nested ones), and the **`precondition`** block (validate before exposing/storing, `condition` + `error_message`, plan-time — the output-side counterpart to a variable `validation`). `ephemeral` output conditions pinned (must come from + be referenced only in ephemeral contexts). Wired into B6's outputs clause + the 1.15 output-type callout, and the I5 deprecation callout; glossary gains `precondition` (output) and Output `type` rows and the `deprecated` row now covers output-specifics. No topic status flipped. · 2026-07-13 — **Source note [[tf-outputs]]** (HCDocs "Use outputs" captured — completes B6's value trio with [[tf-input-variables]] and [[tf-locals]]): the third value block. The **four purposes** (child→parent exposure, root CLI display, cross-config read via `terraform_remote_state` incl. HCP state sharing, passing to automation), `module.<child>.<output>` access, `value` = any expression, and `sensitive`/`ephemeral` outputs (same store-in-state + `-json`/`-raw`-leak caveats as variables; `ephemeral` output is child-modules-only). New **Outputs** nav group mirroring the docs sidebar. Wired into B6 reference #2's outputs link; glossary Output-value row now dual-sourced and expanded. No topic status flipped. · 2026-07-13 — **Source note [[tut-locals]]** (HCTut "Simplify Terraform configuration with locals" — Configuration Language collection #3): hands-on for [[tf-locals]]/[[tf-block-locals]]. Two patterns worth keeping: a `name_suffix` local that de-duplicates interpolated resource names, and the **`merge(var.resource_tags, local.required_tags)` minimum-tags enforcement** — empty the user map's default, build `required_tags` from variables, merge with required *second* so users can add tags but never drop `project`/`environment`. Sharpens the [[tf-input-variables]] rule by contrast: **locals may use dynamic expressions and resource arguments; a variable's value must be literal.** Also shows that because locals feed resource *names*, flipping `-var environment=prod` cascades into replacements (17 add / 15 change / 17 destroy), and notes the real-world alternative to hand-merged tags (AWS provider `default_tags`). Wired into B6 reference #2's locals clause. No topic status flipped. · 2026-07-13 — **Source note [[tut-variables]]** (HCTut "Customize Terraform configuration with variables" — Configuration Language collection #2): the hands-on companion to [[tf-input-variables]]/[[tf-block-variable]], on a VPC + load-balancer + EC2 stack (43 resources; architecture diagram downloaded to `assets/tut-variables/`). Walks **every variable type** (string/number/bool, collection list/map/set, structural tuple/object), the `.tfvars`/`*.auto.tfvars` auto-load + `-var`/`-var-file`/env precedence, `${}` string interpolation, and **multiple `validation` blocks** on one variable using `regexall()` to bound an AWS load-balancer name. The distinctive part is the live **`terraform console`** workflow (list indexing, `slice(list, start, end-exclusive)`, map key lookup) — with the gotcha that the console loads config only at startup. Wired into **B6** ref #1 (deeper variables hands-on) and **B7** ref #1 (console + `slice`/`regexall`/interpolation). No topic status flipped. · 2026-07-13 — **Source note [[tut-resource]]** (HCTut "Define infrastructure with Terraform resources" — first tutorial of the **Configuration Language** collection, a different track from AWS Get Started): the value is the explicit **arguments vs attributes vs meta-arguments** taxonomy that [[tf-aws-create]] uses but never names — arguments configure (required/optional, resource-specific), attributes are provider-assigned read-back values (`type.name.attr`), meta-arguments are Terraform-core behavior knobs (not provider-specific). Also a `random_pet`-for-unique-names pattern, an `aws_security_group` built by reading the Registry argument reference, and `vpc_security_group_ids` as a **list** attribute reference creating an implicit dependency. Wired as a B5 reference. **Nav restructure:** the Tutorials course was a flat AWS-Get-Started numbered list; regrouped by **collection** (`AWS Get Started` / `Configuration Language`) to mirror the tutorial site — same source-fidelity rule as the docs sidebar. No topic status flipped. · 2026-07-13 — **Source note [[tf-block-locals]]** (HCDocs "`locals` block reference" captured — sibling of [[tf-locals]] under the *Locals* sidebar group): the argument spec. Three facts beyond the how-to: **multiple `locals` blocks are allowed in one module and Terraform merges them** as if defined together (split only to group related values visually), each block **must assign at least one value**, and local **names must be valid identifiers**. Values may reference variables, resources, data sources, function outputs, and other locals. Nav sibling under Locals; no path-topic edits needed (B6 already links the how-to). No topic status flipped. · 2026-07-13 — **Source note [[tf-locals]]** (HCDocs "Use locals" captured — completes B6's value trio with [[tf-input-variables]]): short how-to. Plural `locals` block defines, singular `local.<NAME>` references; a local may reference variables, resource attributes, function outputs, and other locals; **module-scoped** (invisible to other modules — pass to a child as an argument to share); and the readability tradeoff (locals obscure where a value originates, so use them for reuse-in-many-places or complex expressions, not every intermediate). Wired into B6 reference #2's locals link; glossary's Local-variable row now dual-sourced. No topic status flipped. · 2026-07-13 — **Source note [[tf-manage-sensitive-data]]** (HCDocs "Manage sensitive data" umbrella captured — the parent behind A6's ephemeral/write-only references): A6 cited the `state/sensitive-data` and `manage-sensitive-data/ephemeral` subpages but not the umbrella that frames the whole decision. Captured the **hide-vs-omit-vs-both** framework, the **version-requirements matrix** (0.15 `sensitive` · 1.10 `ephemeral` arg + block · 1.11 write-only args), the full ephemeral **reference-context** list, and two facts the path didn't state: **`terraform output -json`/`-raw` print `sensitive` values in plain text** (added as a new A6 warning callout — the flags bypass redaction by design), and an **`ephemeral` output is allowed only in child modules, never the root**. Write-only-argument mechanics pinned down: `_wo` + `_wo_version`, provider uses then discards, bump the version to re-send. State-encryption backends recorded for I6 (HCP BYOK, S3 `encrypt`, GCS CSEK/CMEK). Wired as A6 reference #1; glossary gains Ephemeral value, `ephemeral` block/resource, and Write-only argument rows. No topic status flipped. · 2026-07-13 — **Source note [[tf-block-variable]]** (HCDocs "`variable` block reference" captured — the sibling of [[tf-input-variables]] under the *Variables* sidebar group): the argument-level spec behind B6. Full argument surface with defaults — `type` (default `any`), `default` (literal only, no references), `description`, `validation` (`condition` + `error_message`, both required, plan-time), `sensitive` (default `false`, propagates to referencing expressions, still in state), `nullable` (default **`true`**), `ephemeral` (v1.10+, default `false`, with its six valid reference contexts), `const` (default `false`, known-constant-only, unlocks module/provider `source`/`version`), and `deprecated` (v1.15+). Also the **reserved variable names** the path never listed — a label cannot be `source`, `version`, `providers`, `count`, `for_each`, `lifecycle`, `depends_on`, or `locals`. Wired into B6 reference #2 and the I5 deprecation callout; glossary gains reserved-names, `nullable`, `validation` block, and `deprecated` rows (`const` row now dual-sourced). No topic status flipped. · 2026-07-13 — **Source note [[tf-input-variables]]** (HCDocs "Input Variables" / "Use variables" captured): B6 reference #2 was a bare URL pointing at "the precedence table" with no note behind it — now captured in full. The precedence order (highest→lowest): `-var`/`-var-file` + HCP Terraform vars → `*.auto.tfvars`(lexical) → `terraform.tfvars.json` → `terraform.tfvars` → `TF_VAR_` env vars → the `default` argument. Three facts beyond the basic B6 hands-on: **undeclared-variable handling differs by source** (env var silently ignored · `.tfvars` file → warning · `-var` on CLI → error); `sensitive` hides from output but **still stores in state**, and `ephemeral` is the argument that omits it from state/plan; and the **variable lifecycle** finding — most variables evaluate at plan time, but using one in a module/provider `source`/`version` requires the variable's **`const` attribute** so it evaluates at **init** (dovetails with the dynamic-module-sources + `const` work already folded into I4). Wired the capture link into B6 reference #2. No topic status flipped. · 2026-07-10 — **Source note [[tf-block-resource]]** (HCDocs "`resource` block reference" captured — the full argument surface): the real find is **`action_trigger`**, a `lifecycle` rule that fires the Terraform 1.14 **`actions`** the path already covered in A1. It had **zero mentions** anywhere in the notes. Verified on **v1.15.6**: only four events exist — `before_create`, `after_create`, `before_update`, `after_update` — and **there is no destroy event** (`before_destroy` → `Error: No events specified`), so destroy-time work still means a destroy-time provisioner. Found a **docs bug** while writing it up: the page's own spec snippet names the argument `conditions`, its configuration model and complete example name it `condition` (with an `<EXPRESSSION>` typo); Terraform rejects `conditions` as `Unsupported argument`, so `condition` is correct. Also **corrected [[meta-arguments-lifecycle]]**, which listed four `lifecycle` rules — there are **seven** (`action_trigger`, `create_before_destroy`, `prevent_destroy`, `ignore_changes`, `replace_triggered_by`, `precondition`, `postcondition`), the last two being A2's condition blocks rather than standalone constructs. Two corroborations recorded: `lifecycle` takes **literals only** because "it affects how Terraform constructs and traverses the dependency graph" and is processed before expressions are evaluated (the mechanism behind TID's "known early" rule), and `prevent_destroy` "doesn't prevent Terraform from destroying the resource **if you remove the resource configuration**" — confirming TID's third objection, and now a second escape hatch alongside the `removed`-block default. Folded into **A1** (`action_trigger` under the `actions` callout) and **I2** (scope line + a seven-rules callout); glossary gains `action_trigger`. No topic status flipped. · 2026-07-10 — **Correction: the `removed` block DESTROYS by default** (source notes [[tf-destroy-resource]] + [[tf-block-removed]] captured). Four of our own documents said a `removed` block drops a resource from state *without* destroying it — the path's I7 callout, [[tf-resources]], the [[meta-arguments-lifecycle]] topic page, and TID Ch2 §2.9. On current Terraform that is **backwards and dangerous**: it describes `lifecycle { destroy = false }`, which is the opt-*out*. Verified on **v1.15.6** with `terraform_data`: a bare `removed` block plans `1 to destroy`; with `destroy = false` it plans `0 to destroy` and warns the object "will no longer be managed by Terraform, but will not be destroyed". Root cause is **version drift, not a book error** — the **v1.7.x** and **v1.11.x** docs both say the object "should not be destroyed", so `removed` began forget-only and the `destroy` argument arrived later defaulting to `true`. TID's row is now flagged with a version box rather than rewritten. Two docs bugs recorded: the current page **contradicts itself** (its opening sentence says `removed` acts "without changing the underlying infrastructure", its `lifecycle` section says the default destroys), and `lifecycle` is documented **required** yet a `removed` block without it validates and plans fine. Left one `❓ Unverified` in [[tf-block-removed]]: whether a missing `lifecycle` means `destroy = true` or the block is ignored and the object dies simply for being absent from config — the plan's reason line hints at the latter, and the outcome is identical either way. Also new from the destroy how-to: `terraform destroy` **does not edit your configuration**, so `apply` rebuilds; `-target` scopes a destroy; `terraform validate` is the way to find dangling attribute references before deleting a block; and a **destroy-time provisioner** survives block deletion by moving into the `removed` block, which then *requires* `destroy = true` to fire. Glossary gains a `removed` block row. No topic status flipped. · 2026-07-10 — **Source note [[tf-remote-state-data]]** (HCDocs "The `terraform_remote_state` data source" captured): I6 **named** this data source in its scope line and milestone but **no step taught it**, and its security caveat appeared nowhere — a name-dropped gap. The caveat is the headline: reading another config's outputs requires read access to the **entire state snapshot**, because outputs and state are one object and "any user or server which has enough access to read the root module output values will also always have access to the full state snapshot data by **direct network requests**." State holds secrets in plaintext, so this is an exfiltration path, not a formality. HashiCorp's two alternatives recorded — **`tfe_outputs`** on HCP (authorizes outputs *without* full state access), and elsewhere publishing shared values to a store with its own ACLs (SSM / Consul KV / ConfigMap / DNS / S3 + `jsonencode`), wrapped in a **data-only module** so the strategy can be swapped later. Also captured: **only root-level outputs are readable** (nested-module outputs need an explicit passthrough `output` block), `config` takes nested backend blocks as `=` attributes rather than blocks, `defaults` covers an empty or missing output, and the pre-0.12 no-`.outputs` attribute form is dead. Added as I6 reference #3 (TUR renumbered to #4) with a danger callout, plus a matching **A6** callout framing "who may read this state" as the real permission boundary. Glossary gains `terraform_remote_state`, `tfe_outputs`, and **built-in provider** — whose complete contents are exactly these two objects. No topic status flipped. · 2026-07-10 — **Source note [[tf-terraform-data]]** (HCDocs "`terraform_data` resource reference" captured — closes the note behind A1's existing step-1 citation): the built-in provider `terraform.io/builtin/terraform` means **no provider download**, which is why a `terraform_data`-only config can reproduce language behavior offline (used for the DAG experiments). The argument split matters and the path never stated it — changing **`input`** updates in place, changing **`triggers_replace`** *replaces*, which is what re-runs a hosted provisioner. The headline finding is the **`replace_triggered_by` indirection**: that rule takes **only resource addresses** because replacement is decided from the referenced resources' *planned operations*, so "plain data values, such as local values and input variables, aren't valid" — wrapping a variable in a `terraform_data` launders it into something referenceable. Added as an A1 callout. This also **corrected [[meta-arguments-lifecycle]]**, which had attributed the plain-value restriction to the "meta-arguments are known early" rule; that's the wrong reason. Also **merged a duplicate glossary row** — `terraform_data` was defined twice, from TID Ch2 and [[tf-configure-resource]] separately. No topic status flipped. · 2026-07-10 — **Graph coverage gap closed (E5 + B3 + B5)**: a coverage check found `terraform graph` was only ever **name-dropped** — one item in B3's run-on utility list, and a URL in E5's reference #1 — while E5's scope line promised "diagnose the graph" with no material behind it. **E5** now splits that conflated reference into `TF_LOG` (#1) and `terraform graph` (#2), adds a **cycle-diagnosis practice step** (#3) and two callouts: how to read `Error: Cycle` (the error names *members*, `-draw-cycles` shows the *edges*), and the reminder that the graph shows what Terraform knows, not what's true — so a CI-only race points at a missing `depends_on` (I1) before `-parallelism`. New verified fact on **v1.15.6**: `-draw-cycles` **without** an explicit `-type=` is *silently ignored* (no warning, exit 0), and the default resources-only graph renders only **one** of the two cycle edges, making the cycle invisible rather than merely unhighlighted — folded into [[tf-cmd-graph]] and [[dependency-graph]]. **B3** and **B5** now link `[[tf-cmd-graph]]` / `[[dependency-graph]]` instead of dangling. No topic status flipped. · 2026-07-10 — **Topic page [The dependency graph](topics/dependency-graph.md)** + source notes [[tf-meta-depends-on]] and [[tf-cmd-graph]] (chat question → "will Terraform tell me a `depends_on` is missing?"): **no, and it cannot** — the DAG is built only from expression references plus hand-written `depends_on`, so an unseen dependency doesn't exist to Terraform. Verified locally on **v1.15.6** with `terraform_data` (no provider plugin): deleting a `depends_on` silently drops the edge from `terraform graph` while `terraform validate` still reports `Success!`. Failure lands at apply as a **race** (passes locally, fails in CI, or fails once then succeeds), or as a semantic non-failure (the docs' EC2-can't-reach-S3 example). Three investigation routes recorded: `terraform graph` (+ `-type=`, `-draw-cycles`, `-plan=`), the **state file's `dependencies` array** (transitive edges materialized; this orders destroy), and the behavior-vs-data reading test. Counterweight captured: the docs call `depends_on` a **last resort** that forces more `(known after apply)` and replaces more resources than necessary, "especially likely" for modules. Also **corrected [[tf-meta-arguments]]'s block table** — the index page names only `resource` for `depends_on`, while its reference page lists `check`/`data`/`ephemeral`/`module`/`output`/`resource` (+ `component` in Stacks); that's the *second* confirmed undercount on the index page, after `count` omitting `data`. Wired as I1 references #3–4 with a new danger callout; glossary gains hidden dependency, `depends_on` cost, and `terraform graph`. No topic status flipped. · 2026-07-10 — **Source note [[tf-meta-arguments]]** (HCDocs "Meta-arguments" index captured): the definition of the class — built into Terraform core, *not* the provider, so every resource type supports all six regardless of provider, and most work on `module` blocks too. Two facts the path didn't state, both now folded into I1 as a warning callout: **`count` and `for_each` cannot both appear in the same `resource` or `module` block**, and `count` **is** legal on `data` blocks (the index page omits `data` from its `count` list while listing it for `for_each`; the [`count` reference](https://developer.hashicorp.com/terraform/language/meta-arguments/count) names `data`/`ephemeral`/`module`/`resource` plus `list` in query configs — a docs omission on the index, verified 2026-07-10, cached at `cache/web/tf-meta-count.txt`). Also precise: `depends_on` waits for **all** actions on the dependency **including read operations**. Wired as I1 reference #2 (later references renumbered); glossary's "Meta argument" row corrected — it listed only `provider`/`depends_on`/`lifecycle` and now names all six — plus a new `count` vs `for_each` row. Three guessed doc URLs were caught and corrected against the page's real hrefs (`meta-arguments/provider`, `meta-arguments/providers`, and the `count` tutorial at `tutorials/0-13/count`). No topic status flipped. · 2026-07-10 — **Suppressing the implied empty default provider** (chat question → verified against [Providers Within Modules](https://developer.hashicorp.com/terraform/language/modules/develop/providers) + [terraform#35781](https://github.com/hashicorp/terraform/issues/35781); cached at `cache/search/implied-empty-default-provider.md`): there is **no** flag or argument that suppresses it, but it is **lazy** — the "not properly configured" error fires only when a resource binds to it, so an unused empty default is inert. Two mitigations recorded (one unaliased `provider` block, the documented fix; or `provider =` on every `resource`/`data`/`module`, airtight but fragile), plus the correction that **`providers = {}` does not disable inheritance** — an explicit `providers` map overrides inheritance only for the providers it enumerates. Added as a "Can you stop Terraform creating it?" subsection in [[tf-provider-block]], folded into the I8 warning callout and the [Providers topic page](topics/providers.md); glossary's default-provider-configuration row notes the laziness. No topic status flipped. · 2026-07-10 — **Source note [[tf-provider-block]]** (HCDocs "`provider` block reference" captured — note the **page moved**: `language/providers/configuration` → **`language/block/provider`**; I8's reference #1 URL was stale and is now fixed): the configure half of declare-vs-configure, with three facts the path didn't state. The unaliased `provider` block **is** the default configuration; alias every block and Terraform invents an **implied empty default** that unqualified resources silently bind to (new I8 warning callout). Provider *configuration* inherits into child modules but **`source`/`version` do not** — each child redeclares `required_providers`, and an aliased config needs `configuration_aliases = [aws.west]` to receive `providers = { … }`. The `version` argument **inside** a `provider` block is **deprecated**. Also: provider arguments accept expressions over plan-time-known values only (input variables yes, computed attributes no). Extended the [Providers topic page](topics/providers.md) (now 6 sources) with a "default configuration is never absent" section; glossary gains default provider configuration, `configuration_aliases`, and provider-config inheritance, and the alias row is now dual-sourced. No topic status flipped. · 2026-07-10 — **Source note [[tf-dependency-lock]]** (HCDocs "Dependency Lock File" captured): the lock file records **provider selections only** — module versions are never locked, so an exact constraint is the only way to pin a module; `init` re-selects the locked version unless `-upgrade` re-resolves; checksum verification is **trust on first use**, with two traps (a first install from a mirror records only your platform's hashes and breaks every other platform; signed origin-registry checksums record all platforms at once); the `zh:` (zip-archive SHA256, registry-protocol legacy) vs `h1:` (package-contents SHA256, preferred) migration explains the drip of new hashes into diffs; and the four lock-file diff shapes (new provider · upgraded version · new checksums · removed provider, the last requiring the dependency gone from **both config and state**). Wired as B3 reference #3 (CLI commands renumbered to #4) and used to extend B3's cross-platform-checksum callout; glossary gains trust-on-first-use, `zh:` vs `h1:`, the `constraints` field, and `terraform providers lock`. No topic status flipped. · 2026-07-09 — **Source note [[tf-configure-resource]]** (HCDocs "Configure a resource" how-to captured): the three config steps (declare type+name→address, provider args, meta-args), the provider-defined **`timeouts` block** (per-operation duration strings), the meta-argument map (`precondition`/`postcondition`→A2, `provider`→I8, `depends_on`→I1, `replace_triggered_by`→I2), and **built-in / local-only resources** (`terraform_data`→A1; `tls_private_key`/`random_id`/`tls_self_signed_cert` via `terraform.io/builtin/terraform`, with the plaintext-in-state caveat→A6). Wired as a B5 reference; glossary gains the `timeouts` block and local-only resources. No topic status flipped. · 2026-07-09 — **Source note [[tf-providers]]** (HCDocs "Providers" language hub captured): the full provider-tier ladder (Official / Partner Premier / Partner / Community / Archived + namespaces), provider installation (HCP-every-run vs CLI-on-init, `plugin_cache_dir`), the lock-file pointer, and private-registry follow-up-request auth via `.netrc` / `NETRC`. Blended into the [Providers topic page](topics/providers.md) (now 5 sources) with a trust-tiers subsection; glossary gains the tier ladder, plugin cache, and `.netrc`. Feeds B5 (tiers) and B2/E5 (install/cache). No topic status flipped. · 2026-07-09 — **Source note [[google-provider]]** (Terraform Registry Google-provider overview captured): Official tier co-maintained by Google + HashiCorp, `hashicorp/google` v7.39.0 / 2026-06-30, the project/region/zone scoping trio, the ADC-first credential precedence (gcloud ADC → key file → env vars → access token → impersonation), and GCP-specific args (`user_project_override`, `billing_project`, no `default_tags` equivalent). Extended the **[Providers](topics/providers.md)** topic page with an AWS-vs-GCP configuration/authentication comparison (now a 4-source page). No topic status flipped. · 2026-07-09 — **Source note [[tf-resources]]** (HCDocs "Create and manage resources" overview captured): resource definition + meta-arguments framing, the write→init→apply workflow, and the canonical five-operation apply list (create / destroy-orphaned / update-in-place / destroy-and-recreate-on-forced-new / update-state) that maps onto the `+`/`-`/`~`/`-/+` plan symbols, plus the manage-over-time moves (modules, `moved`, `removed`, destroy). Hub page — already covered by [[tf-config-syntax]]/[[tf-aws-create]]/Ch3/Ch5; wired `[[tf-resources]]` into B5 reference #2. No topic status flipped. · 2026-07-09 — **Source note [[aws-provider]]** (Terraform Registry AWS-provider overview captured): registry metadata (Official tier, `hashicorp/aws`, latest v6.54.0 / 2026-07-08, ~1,678 resources + 668 data sources across ~150 services), the canonical example-usage config, the six-step credential precedence order (provider params → env vars → shared credentials file → shared config file → container creds → instance profile), the "never hard-code credentials" warning, and key provider arguments (`region`, `assume_role`, `default_tags`, `skip_credentials_validation`, `max_retries` default 25). New **Terraform Registry** source course (#4). Wired `[[aws-provider]]` into B5 reference #2 (already cited the registry URL). No topic status flipped (no chapter written). · 2026-07-08 — **B4 lab precision**: replaced B4's generic "*HCL Basics*" lab cite with the direct [KodeKloud **Terraform HCL** free lab](https://kodekloud.com/free-labs/terraform/terraform-hcl) (the dedicated blocks/arguments lab). Verified Krausen's labs have **no** B4 fit (lab 01 = setup → B2, lab 02 = first resource → B2/B5; the repo skips pure HCL syntax), so the thin B4 lab list is correct, not a gap. No topic status flipped. · 2026-07-08 — **Source note [[tf-style-guide]]** (HCDocs "Style Guide" captured): code style (2-space indent, `=` alignment, meta-args first / meta-arg blocks last, naming = descriptive noun + underscores, no type in name, param order for resources/variables/outputs, file-name layout `main.tf`/`variables.tf`/`terraform.tf`/…, `.gitignore` do/don't-commit lists) + workflow style (version pinning, `terraform-<PROVIDER>-<NAME>` module repos, local modules `./modules/`, monorepo tradeoffs, GitHub-flow branching, per-workspace/per-directory environments, state sharing via `tfe_outputs`, secrets stay plaintext in state, `terraform test` vs validation, HCP policy). Already B4's reference #1 pairing — closes the missing note; wired `[[tf-style-guide]]` into B4 step 1. Informs B5/B6/I1/I8/A2/A4/A5 (cross-referenced in the note, no path edits there). No topic status flipped. · 2026-07-08 — **Source note [[tf-config-syntax]]** (HCDocs "Configuration Syntax" — the native HCL low-level syntax — captured): arguments (`name = value`) vs blocks (typed, labeled containers), the argument-vs-attribute terminology split, identifier rules (Unicode + ASCII hyphen, no leading digit), three comment styles (`#` idiomatic, `//`, `/* */`), UTF-8 + LF/CRLF encoding. Already B4's reference #1 — this closes the missing note behind an existing citation and wires `[[tf-config-syntax]]` into B4 step 1. Glossary already had Block / Argument-vs-subblock. No topic status flipped (no chapter written). · 2026-07-07 — **Source note [[tf-cli-commands]]** (Terraform CLI Overview / command index captured as a reference): full subcommand table + global options + `-chdir` + tab-completion + Checkpoint telemetry. Diff vs the path's existing CLI-index callout closed two real gaps — **`terraform graph`** (Graphviz dependency graph) added to the B3 command surface, and **Checkpoint opt-out** (`CHECKPOINT_DISABLE` / `disable_checkpoint*`) added to E5 for airgapped/privacy setups; also wired `-install-autocomplete` into B3. `console` already covered (B7). No topic status flipped. · 2026-07-07 — **Source note [[tf-aws-destroy]]** (AWS Get Started "Destroy infrastructure" captured): two teardown paths — remove-from-config + `apply` (surgical, output → `null`) vs `terraform destroy` / `apply -destroy` (whole workspace, reverse-dependency order, "no undo"). Wired as a B3 reference (the `-` symbol + destroy half of the plan loop). Completes the AWS Get Started create→manage→destroy arc in `docs/sources/`; next track lesson is HCP Terraform (→ A4). No topic status flipped. · 2026-07-07 — **HCTut library diff** (fetched the curated [tutorials collections index](https://developer.hashicorp.com/terraform/tutorials) + the flat `library?product=terraform` catalog, 269 cross-product results): the flat library is mostly Vault/Consul/Nomad tutorials that merely *tag* Terraform — not worth bulk-adding; the path already cites the curated core collections (Get Started, modules, state, cloud, policy, config-language, automation, plugin-framework). Two genuine gaps closed from the "New Tutorials" set: **HCP health assessments** (managed drift detection + continuous validation) → A4 callout with the drift-detection + drift-and-policy tutorials; and the concrete **no-code modules** tutorial URL → wired into E6 step 3 (was a generic step). `check` blocks / continuous validation already covered in A2; Azure/GCP/Docker/OCI Get Started tracks are cloud-swapped duplicates of the AWS track already used — not added. · 2026-07-07 — **Source note [[tf-aws-manage]]** (AWS Get Started "Manage infrastructure" tutorial captured): wired as a reference under B3 (in-place `~` vs `-/+` replace, `-out` warning, dependency-graph ordering), B6 (input variables + outputs, `-var`, `terraform output`), and I4 (registry VPC module block, `module.*` references + state addressing, re-`init`). Version-checked: tutorial pins AWS provider `~> 5.92` + VPC module `5.19.0`; current is provider **6.x** / VPC module **6.6.1** (6.x needs provider `>= 6.0`) — flagged in the note + saved to the `terraform-aws-vpc-module-version` search cache. No topic status flipped (no chapter written). · 2026-07-06 — **Free-alternative resources** (web-verified 2026-07-06): added free substitutes for the paid Krausen core-workflow labs — **KL** (Krausen's *own* open-source [terraform-associate-labs](https://github.com/btkrausen/terraform-associate-labs), cloned locally, free Codespaces; the best free match), **KK** (KodeKloud free labs / Coursera audit), **FCC** (freeCodeCamp YouTube), **CLX** (Collabnix labs) — to the resources table + a "free alternatives" callout under B3 step 2 (HCTut Get Started remains the official free 1:1). KL's 8 labs now wired per-topic as 🧪 callouts: Lab 01→B2, 02→B2/B5, 03→B5/B6, 04→B5/I1, 05→B8/B9 (+B3 callout), 06→I3/I4, 07→B6, 08→I1. · 2026-07-06 — **roadmap.sh/terraform diff**: compared the path against the community roadmap's full topic tree (fetched from the developer-roadmap repo, since the page renders client-side). Path is a superset; only real gaps were the roadmap's named **static-analysis scanners** (Checkov/Trivy-tfsec/KICS) and **Infracost** cost gating → folded both into A5 as a third governance layer (SAST + cost) beside plan-time policy. `taint`/`untaint` appear on the roadmap but are already handled as deprecated→`-replace` in B3. · 2026-07-05 — **Authoritative language-index diff** (loop): diffed the path against the full HCDocs language nav — every construct (actions/query/stack blocks, built-in resources, sensitive, backends, state, import, tests) maps. Only non-covered item was the official **Style Guide** (guidance, not a feature) → added a pointer in B4. No new capability found: **feature surface converged.** · 2026-07-05 — **Authoritative CLI-index diff** (loop): diffed the path against the full HCDocs CLI command index (not memory-picked terms) — every documented command now maps to a callout. Added the two missing: `terraform modules` (I4) and `terraform metadata functions` (B7). · 2026-07-05 — **CLI/cross-cutting completeness pass #7** (loop): provider publishing + GPG signing (E1), `expect_failures` test attribute (A2). · 2026-07-05 — **CLI/cross-cutting completeness pass #6** (loop): provider SDKv2 vs Plugin Framework (E1), `TF_WORKSPACE` (A7), HCP self-hosted agents (A4), Stacks deferred changes (E2). · 2026-07-05 — **CLI/cross-cutting completeness pass #5** (loop): `-chdir` global option (B3), override files `override.tf`/`*_override.tf` (B4), `.terraformignore` (A4). · 2026-07-05 — **CLI/cross-cutting completeness pass #4** (loop): built-in named values `path.*`/`terraform.workspace`/`count.index`/`each.*`/`self`/`terraform.applying` (B7), HCP cost estimation + run triggers + notifications (A4). · 2026-07-05 — **CLI/cross-cutting completeness pass #3** (loop): full `state` subcommand set incl. `pull`/`push`/`replace-provider` (I7), `providers mirror` command (E5), debug env vars `TF_LOG_PATH`/`TF_LOG_CORE`/`TF_LOG_PROVIDER` + `crash.log` (E5). · 2026-07-05 — **CLI/cross-cutting completeness pass #2** (loop): backend catalog + `init` flags + locking flags (I6), JSON config syntax `.tf.json`/`.tfvars.json` (B4), `configuration_aliases` (I8), automation env vars (A3), `terraform login`/`logout` (A4), provider `filesystem_mirror`/`network_mirror` (E5). · 2026-07-05 — **CLI/cross-cutting completeness pass** (loop): grepped the path for capabilities a release-timeline doc wouldn't surface. Added: full CLI command surface + `fmt`/`validate`/`show`/`output` (B3), `apply -replace` superseding deprecated `taint`/`untaint` (B3), `force-unlock` (I6), provisioner `when=destroy`/`on_failure`/`self`/`connection` (A1), `dev_overrides` for local provider dev (E1). Added `taint`→`-replace` to the feature-history deprecations table. · 2026-07-05 — **Closed the last deferred niche items**: `provider_meta` (E1), global provider plugin-cache locking (E5), `terraform{} experiments` (E5), `-target-file`/`-exclude-file` (E3). Correction: `-exclude-file`/`-target-file` are OpenTofu 1.10, not 1.9. Nothing usable now deferred. · 2026-07-05 — **Completeness re-audit under widened standard** (goal is learn-Terraform-completely, not cert-only; see [[feature-coverage-matrix]] standard note): folded in every remaining *usable* capability — builtin provider funcs `encode_tfvars`/`decode_tfvars`/`encode_expr` (A1), cross-platform provider checksums (B3), `.tftest.hcl` external vars (A2), `.tofu`/`.tofurc` extensions (E3), OpenTofu MCP server (E6), `rpcapi` + high-cardinality note (E5), ephemeral vars/outputs explicit (A6), and a note that B7 delegates the full ~150-function list to HCDocs. Only unreleased + pure-perf items now excluded. · 2026-07-05 — **Feature-history recheck #2** (after building [[opentofu-feature-history]] + a loop audit of both docs): closed 5 more path gaps — `import` `for_each` + `-refresh-only` (I7), `nullable` (B6), `cloud` block (A4), OpenTofu OpenTelemetry tracing (E5). · 2026-07-05 — **Feature-history reconciliation** (see [[feature-history]]): wrote a full version-by-version feature catalogue (0.11–1.15), which surfaced 1.12–1.14 features the 2026-07-04 matrix predated; closed the gaps — `actions` block (A1), list resources + `terraform query` (B8), cross-object variable validation + `test -parallelism` (A2), `templatestring()` + short-circuit operators (B7), OCI backend (I6), import `identity` (I7). · 2026-07-04 — **Full feature audit** (see [[feature-coverage-matrix]]): built a complete Terraform+OpenTofu feature→topic matrix and closed the gaps it found — ephemeral values/resources + write-only arguments (A6, major), provider-defined functions (B7 use / E1 author), OpenTofu `enabled` meta-arg (I1), native S3 state locking (I6), import `-generate-config-out` (I7), OpenTofu OCI registries (E4). · 2026-07-04 — **Feature-coverage pass** for Terraform 1.15 / OpenTofu 1.12 (see [[tf115-ot112-features]]): folded in dynamic module sources + `const` (I4), variable/output deprecation (I5), output `type` (B6), `convert()` (B7), test-double functions (A2), OpenTofu `destroy = false` (I7), `-json-into` (A3), and expanded the E3 divergence list to 7 items with the "Terraform 1.15 closed some gaps" note. · 2026-07-04 — **Version bump**: current stable is now Terraform CLI **1.15.7** / OpenTofu **1.12.3**; Terraform **1.15** (2026-04-29) closed some OpenTofu gaps (dynamic module sources, deprecation mechanism, inline type conversion, output type constraints, Windows ARM64) — state encryption etc. remain OpenTofu-only (see [[version-facts]]). · 2026-07-04 — **E3 gap closed**: added per-feature references + notes for the four OpenTofu-only divergences beyond state encryption (provider `for_each` 1.9, early variable eval in backend config 1.8, `-exclude` flag 1.9, dynamic `prevent_destroy` 1.12); confirmed `-exclude` is OpenTofu-only (Terraform has no equivalent). · 2026-07-03 — **B1 complete**: Book Ch 1 written (blends [[terraform-intro]], [[terraform-use-cases]], TID Ch1, plus 2026 web research on the IBM/HashiCorp acquisition and current OpenTofu-vs-Terraform guidance); three cross-source topic pages (IaC fundamentals, Core workflow, Providers). Initial path built 2026-07-02 from Terraform Associate 004 + Authoring/Operations Pro exam objectives, then-current Terraform 1.14 / OpenTofu 1.12, and the top books.
