...
!!! warning "🔄 Revisit (2026-07-17) — Ch 6 undersells output `type`"
    `docs/book/ch06-input-variables-outputs-locals.md` says typing outputs "improves matching across validation, plan, and apply". True but weak: it turns out to change the **type callers receive** for `module.<NAME>` under `count`/`for_each` (see the note below). Ch 6's treatment should be reworked around that, and the "Prefer typing the outputs of any module others consume" line should say *why*. B6 counts as done again once Ch 6 carries it.

!!! note "📌 Output `type` (Terraform 1.15) — it changes the type your callers receive"
    Terraform **1.15** added a `type` attribute to `output` blocks — outputs can now carry the same type constraints (and documentation value) variables always have. Prefer typing your module outputs, and for a sharper reason than documentation: **a fully-typed module interface changes the shape of `module.<NAME>` itself.** With `count`/`for_each`, Terraform hands callers a `tuple`/`object` when it can't pin the output types down, and a real `list`/`map` when it can (`GetModule` branches on "does the module fully define all output types"). Verified on 1.15.8 — adding `type = string` to a module's one output flips `module.c` (count) from `tuple([obj, obj])` to `list(object(...))`, and `module.f` (for_each) from `object({...})` to `map(object(...))`. **Resources have no such switch** — they are always structural, because a provider's dynamic-typed attributes can diverge per instance (Ch 7 §3). A module's interface is yours to declare, so declare it. **OpenTofu has no typed outputs** (TF-only as of OT 1.12.4), so module references there stay structural. Evidence: [[conditional-branch-evaluation]]. (See [[tf115-ot112-features]]; the full `output` argument spec is in [[tf-block-output]].)
...
!!! tip "📌 Typing outputs is part of the module's interface, not its docs"
    A module's **outputs** deserve `type` constraints as much as its inputs do (TF **1.15+**, B6). Beyond validation, a fully-typed output set is what lets Terraform give your callers a **`list`/`map`** for `module.<NAME>` under `count`/`for_each` instead of a `tuple`/`object` — verified on 1.15.8, and the switch is exactly "does the module fully define all output types". A caller who wants to `[*]`-splat or pass your module's instances into something expecting a collection feels the difference. Untyped outputs silently push a structural type onto everyone who consumes you. **OpenTofu has no typed outputs** as of 1.12.4, so a module written for both stays structural there. Evidence: [[conditional-branch-evaluation]]; the type-system half is Ch 7 §2-§3.
