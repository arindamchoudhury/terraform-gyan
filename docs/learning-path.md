...
1. **Reference — [HCDocs "Provider Configuration" + aliases](https://developer.hashicorp.com/terraform/language/providers/configuration)** (~40 min) — alias syntax and passing providers to modules. A module that needs a caller to hand it multiple provider configs declares **`configuration_aliases`** inside its `required_providers` — the explicit contract for the `providers = { … }` argument on the module block. (See [[feature-history]].)
