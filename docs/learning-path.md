...
4. **Interactive — HCTut ["Create resource dependencies"](https://developer.hashicorp.com/terraform/tutorials/configuration-language/dependencies)** (~10 min) — the `depends_on` lab: an Elastic IP implicitly ordered behind one EC2 instance while a second instance runs in parallel, then an explicit `depends_on` on both a `resource` and a `module` block. Notes: [[tut-dependencies]] — read the apply and destroy logs, not just the config. Interleaved `Creating...` lines mean Terraform thinks those nodes are independent, so the log is a free dependency check. It also names the **second** cost of `depends_on` that the reference page omits: longer apply wall-clock time from serialized work, on top of the degraded plan. ⚠️ The example config pins AWS provider `~> 4.17.1` and writes `vpc = true` on `aws_eip`; that argument was **removed in provider v6.0.0** — use `domain = "vpc"` in anything you write yourself.
5. **Practice — inspect the DAG** (~20 min) — run `terraform graph` on a config with one implicit edge and one `depends_on`, then delete the `depends_on` and watch the edge vanish while `validate` still passes. Notes: [[dependency-graph]], [[tf-cmd-graph]].
