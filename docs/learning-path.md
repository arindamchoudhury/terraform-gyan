...
1. **Reference — [HCDocs "Data Sources"](https://developer.hashicorp.com/terraform/language/data-sources)** (~20 min) — the block syntax and when data is read (plan vs apply). Notes: [[tf-data-sources]] — the plan-vs-apply deferral rules, custom conditions, `count`/`for_each` on data blocks, aliased providers.
