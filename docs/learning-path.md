...
!!! danger "📌 Lifecycle rules are not in state — except `create_before_destroy` (added 2026-08-03)"
    ...
    `create_before_destroy` is exempt because it decides destroy *ordering*, which must still work once the config is gone; state v4 stores it per instance. It also **propagates**: enabling it on A enables it on everything A depends on, written to state, and those resources cannot override it back to `false` (that would imply a graph cycle). So one `create_before_destroy = true` can change the replacement strategy of resources you never edited — and CBD is only safe where old and new objects can coexist. Both verified against Terraform **v1.15.8** source. (See [[tf-meta-lifecycle]], [[meta-arguments-lifecycle]].)
