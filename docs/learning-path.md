...
!!! warning "📌 `sensitive` leaks through `terraform output -json` / `-raw`"
    `sensitive` redacts values in normal CLI logs and the HCP UI, but **`terraform output -json` and `terraform output -raw` print them in plain text** — the flags are designed to feed automation, so they deliberately bypass redaction. Combined with the fact that `sensitive` values are stored in state/plan anyway, this is why `sensitive` alone is *hiding*, not *protecting*. Use `ephemeral` (+ write-only args) to keep a secret out of state entirely. (See [[tf-manage-sensitive-data]].)
