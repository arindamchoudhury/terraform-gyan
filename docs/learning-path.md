...
2. **Interactive — experiment** (~45 min) — add `create_before_destroy` to a resource and watch the plan reorder to `+` before `-`. Add a second resource that the first depends on and confirm CBD propagated to it without being written there.
