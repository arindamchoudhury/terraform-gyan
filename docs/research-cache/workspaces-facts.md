...
- **HCP Terraform workspace** — a much richer unit: its own Terraform configuration, its own variable values, state data, run history, and settings/RBAC. Closer to "a whole deployment," and the unit HCP Terraform's role-based access control is built around. **Proprietary, not open source** — it's a feature of the HCP Terraform hosted SaaS platform (and its self-hosted twin, Terraform Enterprise), both closed-source and paid. No single open-source product replaces it, but the two halves have separate open answers: **Terragrunt** covers the isolation half (per-unit state backend, per-unit inputs, and a per-unit `iam_role`), while **Atlantis** or ordinary CI covers the platform half (pull-request runs, approvals, audit trail). The topic page [[workspaces]] lays out the split. *(Corrected 2026-07-27 — this originally read "there is no open-source equivalent; the closest open option is a directory-per-env layout with your own remote backend", which undersold Terragrunt.)*
