...
Terraform is an IaC tool for building, changing, and versioning infrastructure safely and efficiently. It isn't the only one — Pulumi is another vendor-agnostic framework (using general-purpose languages instead of a custom DSL); AWS CloudFormation is the vendor-specific equivalent, tied to one cloud (GCP's Deployment Manager filled the same role until Google retired it on 31 March 2026, replacing it with Infrastructure Manager — which runs Terraform under the hood). Terraform's position: broad vendor coverage, a purpose-built declarative language, and (as of 2026) the largest and most mature provider ecosystem of the group.
...
The tension had been building for a while before it became public: HashiCorp updated its contributor documentation in 2021 to say it had paused routine review of community pull requests to Terraform Core, citing understaffing — framed as temporary at the time, but an early signal in hindsight of the shift to come. In August 2023, HashiCorp relicensed Terraform (and several other products) from the open-source MPL to the Business Source License (BSL), starting with Terraform 1.6. BSL is a "shared source" license: the code stays publicly viewable and auditable, but usage is restricted — specifically, it blocks offering the licensed software to third parties on a hosted or embedded basis that competes with HashiCorp's own products. Ordinary internal use, by individuals or companies, remains permitted.

Worth being precise about the blast radius, because this is widely misremembered as "Terraform went closed": only HashiCorp's **core products** moved to BSL. Terraform's **providers** stayed MPL, as did many individual HashiCorp libraries. The AWS provider you download during `init` is open source under the same license it always had. The relicense covered the CLI and its engine, not the plugin ecosystem built around them.

The community's response was fast and organized: a manifesto published that same month — co-signed by 140+ companies, 11 software projects, and 700+ individual developers, and still accumulating signatures since — asked HashiCorp to reconsider — and warned that a fork would follow if it didn't. Much of the concern centered on how dependent Terraform was on third-party-built tooling — Gruntworks' Terratest and Terragrunt among them — that the community felt the relicense put at risk. HashiCorp didn't reconsider. **OpenTofu** launched as that fork, backed by Scalr, env0, Spacelift, and Harness committing roughly 18 developers for five-plus years, and placed under the Linux Foundation specifically so no single company can control its direction again.
...
That compatibility runs in one direction more freely than the other, and it's the thing to weigh before adopting either. Moving a codebase **Terraform → OpenTofu** is easy: OpenTofu reads what Terraform wrote. Moving **OpenTofu → Terraform** is easy only until you use an OpenTofu-only feature. Encrypt your state, or write a provider `for_each`, and there is no Terraform equivalent to migrate back to — you'd have to unwind the feature first. The second asymmetry runs the other way: OpenTofu has to independently re-implement each new Terraform feature, so it lags behind on the newest ones rather than shipping them the same week. "Superset" describes the feature sets as they stand today, not the release cadence.

One concrete side-effect of the relicense: several open-source package managers, Homebrew included, stopped shipping Terraform versions beyond **v1.5.7** — the last MPL release — while continuing to carry OpenTofu.

Two events since the initial fork are worth knowing as of 2026:

!!! info "IBM now owns HashiCorp"
    **IBM announced its $6.4B acquisition of HashiCorp in April 2024**; the deal closed **February 2025** after US/UK regulatory review. Terraform is now developed under IBM.

!!! info "Current 2026 guidance, per multiple independent comparisons"
    OpenTofu is increasingly the lower-risk default for a *new* project — OSI-approved license, multi-vendor governance, full provider compatibility, plus the CLI features listed above. Price that against the one-way door: adopting an OpenTofu-only feature is what makes the choice hard to reverse, so decide deliberately rather than drifting into it. Staying on Terraform still makes sense if you're already invested in HCP Terraform, use Terraform **Stacks** (a Terraform-exclusive capability that lives in HCP Terraform, not the open CLI — covered in E2), or work somewhere procurement specifically requires HashiCorp as vendor. Existing Terraform investment on its own isn't a reason to migrate — evaluate OpenTofu at your next new-project or compliance decision point instead.
...
- **"OpenTofu is a lesser Terraform."** By 2026 it's the reverse in several respects — OpenTofu is a superset of Terraform's open-source feature set, not a stripped-down copy (see [[version-facts]]). The gap that remains (mainly Stacks) is deliberately HCP-Terraform-exclusive, not a maturity gap in the open CLI. The honest qualifier: superset of what has *shipped*, not of what ships *next* — OpenTofu re-implements each new Terraform feature independently, so it trails on the newest arrivals.
- **"The BSL relicense made the Terraform ecosystem closed source."** Only HashiCorp's core products moved to BSL. Providers stayed MPL, and so did many HashiCorp libraries — the plugin ecosystem you actually depend on never changed license.
...
- HashiCorp's 2023 BSL relicense produced the **OpenTofu** fork (Linux Foundation, MPL 2.0); as of 2026, OpenTofu is a superset of Terraform's *shipped* open-source CLI feature set, and is the commonly recommended default for new projects unless you need HCP-Terraform-exclusive features like Stacks. Migration is asymmetric — Terraform → OpenTofu is easy, the reverse is easy only until you adopt an OpenTofu-only feature.
