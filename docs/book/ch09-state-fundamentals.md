...
!!! info "OpenTofu — client-side state encryption"
    OpenTofu ships opt-in **state encryption**, configured in the `terraform` block or through `TF_ENCRYPTION`. It covers "state and plan files at rest, both for local storage and when using a backend" — so the plaintext password you found above would be encrypted in the file itself, not merely in whatever the backend does underneath. Terraform's open-source CLI has no equivalent as of 1.15 (nor in 1.16.0-rc1); it relies on the backend's at-rest encryption (S3's `encrypt`, GCS's customer-managed keys) or on HCP Terraform. If encrypted-at-source state is a hard requirement, that is a real reason to evaluate OpenTofu.

    **Learn the exit before you take the entrance.** Encryption reads like a one-way door, and it is not: OpenTofu registers an **`unencrypted` method** alongside `aesgcm`, and has since the feature shipped in 1.7.0. It is what a `fallback` block uses to read still-plaintext state while you roll encryption out, and what you migrate *to* if you ever need to decrypt. It appears in **no changelog and no release blog** — it is visible only in `internal/encryption/default_registry.go` — so it is entirely possible to adopt encryption without learning that the way back exists. The registered set as of 1.12: key providers `pbkdf2`, `aws_kms`, `gcp_kms`, `azure_vault`, `openbao`, `external`; methods `aesgcm`, `external`, `unencrypted`. Setup and the rollout sequence are E3's subject; what belongs here is that the door opens both ways.
