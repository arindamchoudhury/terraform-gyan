...
!!! warning "Omitting the index is not shorthand for “the first one”"
    From the same page: *"Omitting an index when addressing a resource where `count > 1` means that the address **references all instances**."* The page states that rule for `count` only, and says nothing of the kind under `for_each`. The four-row table in the next subsection measures a `for_each` resource, `aws_s3_bucket.shard` with the keys `"a"` and `"b"`, and it behaves the same way, so read the rule as being about instance keys rather than about `count`.
...
- **Terraform's own errors.** A crash before state was saved, a mid-run credential expiry, a corrupted write. The dangerous shape is a resource that was **created but never recorded**, so the next run plans to create it again. What follows depends on the resource: a genuine duplicate where the provider allocates the identifier, or a hard failure where the name must be unique. Measured in `labs/chapter16/section8/lost-record`, which simulates the lost write with a `state rm` and then re-applies, the second outcome arrives as `Error: creating S3 Bucket (ch16-dup-demo): BucketAlreadyExists`. Neither outcome is recoverable by re-running: the way back is an `import` block, as in section 6.
