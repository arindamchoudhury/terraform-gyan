...
> 🔄 **Revisit (2026-08-15)** — `opentofu#2630` was closed as **completed** on **2026-08-13**, carrying the labels `accepted`, `needs-rfc` and `core-team`. This paragraph and the learning path both still describe it as an *open* request, which may now be stale. What is **not** established: whether a `convert` function actually shipped, and in which release. It is not in the local `repos/opentofu` checkout, but that checkout is pinned at 2026-07-13 and predates the closure, so it settles nothing. Current stable at time of writing is OpenTofu **1.12.5** (2026-07-21), also before the closure. **Re-verify against a fetched `main` and the 1.13 changelog before treating `convert()` as available**, and sweep this chapter, `learning-path.md` (B7) and any glossary entry together. Typed outputs are no help either, since `type` on an `output` is also Terraform 1.15+ and OpenTofu rejects it. For primitives and collections the fixed casters (`tostring`, `tonumber`, `tolist`, `toset`, `tomap`) cover the same ground in both tools, so the gap is specifically **object and tuple schemas**.
