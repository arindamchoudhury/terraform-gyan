...
!!! info "OpenTofu — no `convert()`, but the behavior is still reachable"
    ...
    What OpenTofu can't do is apply an arbitrary schema **inline**, mid-expression, with no declared boundary. That is exactly what [opentofu#2630](https://github.com/opentofu/opentofu/issues/2630) asks for, and its motivating case is coercing `jsondecode`/`yamldecode` output.

!!! info "OpenTofu — `convert` is coming in 1.13, and is in no released version yet"
    Settled from source on **2026-08-15**, after [opentofu#2630](https://github.com/opentofu/opentofu/issues/2630) closed as **completed** on 2026-08-13.

    **OpenTofu `main` has a `convert` function.** It is registered at `internal/lang/functions.go:153` as `makeConvertFunc()`, implemented in `internal/lang/type_conversion.go:109`, and described in-tree as *"`convert` attempts to convert the given value to match the given target type constraint"* — two parameters, the value and the type constraint. Same shape as Terraform's, and it uses the same `customdecode` capsule trick to accept a bare type expression like `list(string)` as an argument.

    **It is in no release.** `git tag --contains` on the introducing commit (`e1bd61f3`) returns **nothing**, and the newest tag in the repository is **v1.12.5**. The changelog entry sits under **`## 1.13.0 (Unreleased)`**, from [opentofu#4449](https://github.com/opentofu/opentofu/pull/4449).

    So the sentence above stays correct for every OpenTofu you can install today (≤ **1.12.5**), and becomes wrong when **1.13.0** ships. Re-check at that point. The same PR also added a family of `assume…` functions for hinting the eventual type of an unknown value, which is the other half of what the issue asked for and has no Terraform equivalent. Typed outputs are no help either, since `type` on an `output` is also Terraform 1.15+ and OpenTofu rejects it. For primitives and collections the fixed casters (`tostring`, `tonumber`, `tolist`, `toset`, `tomap`) cover the same ground in both tools, so the gap is specifically **object and tuple schemas**.
