...
|---|---|
| Numeric | `min` `max` `abs` `ceil` `floor` `pow` `parseint` `signum` |
| String | `format` `join` `split` `replace` `substr` `lower`/`upper`/`title` `trimspace` `startswith`/`endswith` `regex`/`regexall` |
| Collection | `merge` `lookup` `coalesce` `concat` `flatten` `keys`/`values` `contains` `distinct` `length` `toset` `setproduct` `zipmap` `slice` `sort` `alltrue`/`anytrue` `one` |
| Encoding | `jsonencode`/`jsondecode` `yamlencode`/`yamldecode` `csvdecode` `base64encode`/`base64decode` `urlencode` |
| Filesystem | `file` `fileexists` `fileset` `templatefile` `abspath` `dirname`/`basename` |
| Date/time | `timestamp` `plantimestamp` `timeadd` `timecmp` `formatdate` |
| Hash/crypto | `sha256` `md5` `bcrypt` `filesha256` `uuid` `uuidv5` |
| IP network | `cidrhost` `cidrsubnet` `cidrsubnets` `cidrnetmask` |
| Type conversion | `tostring` `tonumber` `tobool` `tolist` `tomap` `toset` `try` `can` `type` `sensitive`/`nonsensitive` |
