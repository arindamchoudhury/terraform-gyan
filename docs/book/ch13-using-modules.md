...
An empty top half is the easy case. `aws-ia/vpc/aws` shows what the tab looks like when both halves are populated:

[![The Dependencies tab on aws-ia/vpc/aws at v4.8.0. Under Dependencies, four entries: subnet_calculator (1.0.2) drewmullen/subnets/cidr, subnet_tags (0.0.6) aws-ia/label/aws, tags (0.0.6) aws-ia/label/aws, vpc_lattice_tags (0.0.6) aws-ia/label/aws. Under Provider Dependencies, four entries: aws (hashicorp/aws) >= 5.0.0, aws (hashicorp/aws) >= 3.72.0, aws (hashicorp/aws) >= 4.0.0, and awscc (hashicorp/awscc) >= 0.15.0.](assets/ch13-registry-vpc-dependencies-tab.png)](assets/ch13-registry-vpc-dependencies-tab.png)

Three things to take from it. Calling that module downloads four more, one of them `drewmullen/subnets/cidr` from a third namespace entirely, so the provenance check you ran on the page you are reading does not cover everything `init` fetches. The dependency versions are exact, `1.0.2` and `0.0.6`, because the author pinned them rather than leaving a range. And `hashicorp/aws` appears three times with three different floors, which is not a contradiction: the tab pools declarations from the root module and every submodule, and the registry API confirms the split, with `>= 5.0.0` on the root, `>= 3.72.0` on `modules/calculate_subnets`, and `>= 4.0.0` on `modules/flow_logs/modules/s3_log_bucket`. Whichever of them your call actually reaches, `init` intersects with yours, so the effective floor here is the highest one that applies.
