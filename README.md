...
| Text after the closing quote of an admonition title | error | The title is end-anchored, so `!!! note "title" stray words` matches nothing — the **whole block**, title and body, renders as plain paragraph text. Nested straight quotes (`"a "b" c"`) are legal and are not flagged |
