...
| `section4/` | four `moved` edge cases: both addresses declared at once, a block written backwards, a module call renamed without a re-`init`, and the `count` auto-move that `for_each` does not get | plan-only; three of the four fail on purpose |
