...
| `section4/` | five `moved` edge cases: both addresses declared at once, a block written backwards, a module call renamed without a re-`init`, the `count` auto-move that `for_each` does not get, and a two-hop chain that collapses | plan-only; three of the five fail on purpose |
